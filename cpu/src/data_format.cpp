...
// 找到这个一维向量的最大值
data_type Tensor3D::max() const { return this->data[argmax()]; }
...
// 找到这个一维向量的最大值
data_type Tensor3D::min() const { return this->data[argmin()]; }
...
void Tensor3D::normalize(const std::vector<data_type> mean,
                         const std::vector<data_type> std_div) {
  if (C != 3) return;
...
cv::Mat Tensor3D::opecv_mat(const int CH) const {
  ...
  if (CH == 3) {
    ...
    for (int i = 0; i < length; ++i) {
      ...
      origin.data[p + 2] =
          cv::saturate_cast<uchar>(255 * data[i + length + length]);
    }
  } else if (CH == 1) {
...
void Tensor3D::print_shape() const {
  std::cout << this->name << "  ==>  " << this->C << " x " << this->H << " x "
            << this->W << "\n";
}
...
std::shared_ptr<Tensor3D> Tensor3D::pad(const int padding) const {
  std::shared_ptr<Tensor3D> padded(
      new Tensor3D(C, H + 2 * padding, W + 2 * padding,
                   this->name + +"_padding_" + std::to_string(padding)));
